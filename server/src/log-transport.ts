...
class LogTransport extends Writable {
	...
	constructor(options: TransportOptions) {
		...
		this.options = {
			...
			retentionDays: positiveInteger(options.retentionDays, 'retentionDays'),
			bufferBytes: positiveInteger(options.bufferBytes, 'bufferBytes'),
			...
			httpEventSource: new Map(
				Object.entries(options.httpEventSource).map(([event, source]) => [event, safeToken(source)] as const),
			),
	 ...
	 * 并将连续的相同分类记录合并后交给对应 writer。
	 */
	private async processChunk(chunk: string) {
		this.pending += chunk;
		const lines = this.pending.split('\n');
		this.pending = lines.pop() || '';
		// 同一 chunk 中连续的相同分类先合并，减少 writer 调度和字节计算次数。
		const batches: { date: string; component: string; source: string; data: string; bytes: number }[] = [];
		let lastKey = '';
		...
		for (const line of lines) {
			...
			const source = getSource(
				...
				this.options.httpEventSource,
				this.options.httpBusinessPrefixes,
			);
			const date = getDate(record.time);
			const key = `${component}\0${source}\0${date}`;
			const data = `${line}\n`;
			const batch = batches[batches.length - 1];
			if (batch && key === lastKey) {
				batch.data += data;
			} else {
				batches.push({ component, source, date, data, bytes: 0 });
				lastKey = key;
			}
		}

		for (const batch of batches) {
			// 每个批次只计算一次 UTF-8 字节数，再交给对应 writer 处理轮转。
			batch.bytes = Buffer.byteLength(batch.data, 'utf8');
			const key = `${batch.component}\0${batch.source}`;
			let writer = this.writers.get(key);
			if (!writer) {
				writer = new LogFileWriter({ ...this.options, component: batch.component, source: batch.source });
				this.writers.set(key, writer);
			}
			await writer.appendBatch(batch.date, batch.data, batch.bytes);
		}
	}
}
...
class LogFileWriter {
	...
	/**
	 * 将日志批次加入串行写入队列，并按日期、大小和缓冲阈值安排刷盘。
	 */
	appendBatch(date: string, data: string, bytes: number) {
		...
		this.operation = this.operation.then(async () => {
			...
			await this.appendData(data, bytes);
	...
	private async appendData(data: string, bytes: number) {
		...
		// 只有批次本身超过文件上限时才逐行拆分，常规批次保持一次字符串追加。
		for (const line of data.split('\n')) {
			if (!line) {
				continue;
			}
			const lineData = `${line}\n`;
			const lineBytes = Buffer.byteLength(lineData, 'utf8');
			if (
				this.fileBytes + this.pendingBytes + lineBytes > this.options.maxBytes &&
				this.fileBytes + this.pendingBytes > 0
			) {
				await this.flush();
				await this.rotate();
			}
			this.pending += lineData;
	...
	private async ensureStream() {
		...
		if (!this.index) {
			this.index = await findNextIndex(directory, this.options.source, this.options.maxBytes);
		}
	...
	private async flush() {
		...
		if (!this.stream!.write(data, 'utf8')) {
	...
	async close() {
		...
		await this.flush();
		// 定时器回调可能在等待操作期间追加一次刷盘任务，因此关闭前再等待一次队列。
		await this.operation;
		await this.closeStream();
	}
}

async function findNextIndex(directory: string, source: string, maxBytes: number) {
	const prefix = `${source}.`;
	let highest = 0;
	// 只扫描当前来源的文件，重启后继续使用未满的最高序号文件。
	...
	const currentPath = path.join(directory, `${source}.${String(highest).padStart(4, '0')}.ndjson`);
	try {
...
function fileName(options: WriterOptions, index: number) {
	return `${options.source}.${String(index).padStart(4, '0')}.ndjson`;
}
...
function getSource(
	...
	if (component === 'http') {
		const exactSource = httpEventSource.get(event);
		if (exactSource && sources.has(exactSource)) {
			return exactSource;
		}
		if (event.startsWith('http.request.')) {
			return 'error';
		}
		if (httpBusinessPrefixes.some((prefix) => event.startsWith(prefix))) {
			return 'business';
		}
	}
	return 'default';
...
async function cleanupOldLogs(options: ResolvedOptions) {
	...
	let components;
	try {
		components = await readdir(options.logDir, { withFileTypes: true });
	} catch {
		return;
	}
	for (const component of components) {
		...
		const componentDir = path.join(options.logDir, component.name);
		let dates;
		try {
		...
		for (const dateEntry of dates) {
			if (!dateEntry.isDirectory() || !isDateName(dateEntry.name)) {
				continue;
			}
